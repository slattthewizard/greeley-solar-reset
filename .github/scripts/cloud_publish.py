...
# Order matters: first match wins, so most-specific fragments come first.
TAG_MAP = {
    'detach and reset': 'Detach & Reset', 'detach & reset': 'Detach & Reset',
    'remove and reinstall': 'Detach & Reset', 'removal and reinstall': 'Detach & Reset',
    'reinstall': 'Detach & Reset', 'reset': 'Detach & Reset',
    'insurance': 'Insurance Claims', 'claim': 'Insurance Claims', 'adjuster': 'Insurance Claims',
    'deductible': 'Insurance Claims', 'coverage': 'Insurance Claims',
    'roof replacement': 'Roofing & Solar', 'reroof': 'Roofing & Solar', 're-roof': 'Roofing & Solar',
    'new roof': 'Roofing & Solar', 'roofer': 'Roofing & Solar', 'roofing': 'Roofing & Solar',
    'shingle': 'Roofing & Solar', 'roof leak': 'Roofing & Solar',
    'hail': 'Hail & Storm', 'storm': 'Hail & Storm', 'wind damage': 'Hail & Storm',
    'lightning': 'Hail & Storm', 'microcrack': 'Hail & Storm',
    'cost': 'Cost Guide', 'price': 'Cost Guide', 'worth': 'Cost Guide', 'who pays': 'Cost Guide',
    'out of business': 'Orphaned Systems', 'bankrupt': 'Orphaned Systems',
    'orphan': 'Orphaned Systems', 'warranty': 'Orphaned Systems',
    'removal': 'Panel Removal', 'remove': 'Panel Removal', 'disposal': 'Panel Removal',
    'recycl': 'Panel Removal',
    'inverter': 'Repair', 'not producing': 'Repair', 'not working': 'Repair',
    'repair': 'Repair', 'fix': 'Repair', 'troubleshoot': 'Repair', 'error': 'Repair',
    'lease': 'Leases & Home Sales', 'ppa': 'Leases & Home Sales', 'buyout': 'Leases & Home Sales',
    'selling': 'Leases & Home Sales', 'home sale': 'Leases & Home Sales', 'real estate': 'Leases & Home Sales',
    'buying a house': 'Leases & Home Sales',
    'clean': 'Maintenance', 'maintenance': 'Maintenance', 'inspect': 'Maintenance',
    'snow': 'Maintenance', 'winter': 'Maintenance', 'bird': 'Maintenance', 'squirrel': 'Maintenance',
    'net metering': 'Rules & Utilities', 'xcel': 'Rules & Utilities', 'utility': 'Rules & Utilities',
    'permit': 'Rules & Utilities', 'hoa': 'Rules & Utilities', 'code': 'Rules & Utilities',
}
...
def make_nav_title(title):
    ...
    nav = re.sub(r'\s*[-–—]\s*Greeley Solar Reset.*$', '', nav)
    nav = re.sub(r'\s*\(20\d{2}.*?\)', '', nav)
    nav = re.sub(r'\s*Greeley:?\s*', ' ', nav).strip()
    if len(nav) <= 30:
        return nav
    words = nav.split()
    result = ''
...
def clean_body_links(body):
    body = re.sub(r'https?://greeleysolarreset\.com/', '/', body)
...
def convert_and_write(draft_path, output_path, publish_date, slug):
    ...
    canonical = f'https://greeleysolarreset.com/blog/{slug}/'
